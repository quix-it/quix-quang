 ...
 */
export class QuangGoogleMapComponent implements OnChanges {
  /**
   * Html id of input
   */
  @Input() id: string = ''
   ...
   */
  constructor(private readonly googleMapService: QuangGoogleMapService) {
    this.renderMap()
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.markers?.currentValue) {
      of('')
        .pipe(
          ...
          retryWhen((errors) => errors.pipe(delayWhen((val) => timer(100))))
        )
        .subscribe(() => {
          this.removeMarkers()
          this.createMarkers()
    ...
    if (changes.defaultCenter?.currentValue) {
      if (this._window().google && this._map) {
        this._map.setCenter({
          lat: this.defaultCenter[0],
          lng: this.defaultCenter[1]
        })
      }
    }
  }
